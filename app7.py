    ...
    if isinstance(help_TF_Q, dict):
        ...
        if question=="" or explanation=="" or answer=="":
            ...
            with st.chat_message("assistant"):
                clarification_message = "How can I help you with this True/False question?"
                st.markdown(clarification_message)
            #st.session_state.messages.append({"role": "assistant", "content": clarification_message})
            if query: #st.text_input("Ask for clarification or type your question about this True/False item:"):
                with st.chat_message("user"):
                    st.markdown(query)
                st.session_state.messages.append({"role": "user", "content": query})

                # Retrieve relevant context from the vector database
                query_rag = f"Question: {question}\nExplanation: {explanation}\nAnswer: {answer}\nContext for clarification: {query}" 
                ...
                with st.chat_message("assistant"):
                    answer = st.write_stream(stream_string_data(answer))
