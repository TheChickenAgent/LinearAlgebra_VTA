...
def intro():
    ...
    st.markdown(
        ...
        Please go to the page "User testing" using the sidebar navigation. When done, please fill in the [survey](https://maastrichtuniversity.eu.qualtrics.com/jfe/form/SV_3f9d79itNZlNvFQ).
    
        The different modes are:
        - User testing: tutorial like setting with True/False questions and a help chat.
        - Linear Algebra chat: same as the help chat function in user testing
        - Linear Algebra direct: direct use of the expert model for Linear Algebra questions.
        - Practise TFQ: practice automatically generated True/False questions based on chosen topics with instant feedback.
    """
    )
...
def clean_latex_for_streamlit(explanation: str) -> str:
    """
    Cleans LaTeX input for compatibility with Streamlit rendering inside st.info or st.markdown.
    - Removes align environments
    - Removes & symbols
    - Splits multi-line equations into individual $$...$$ blocks
    - Keeps normal text and inline math unchanged
    """
    # Remove align environments
    explanation = re.sub(r'\\begin{align\*}', '', explanation)
    explanation = re.sub(r'\\end{align\*}', '', explanation)

    # Clean each block math segment ($$ ... $$)
    def split_multiline_block(match):
        content = match.group(1)
        # Remove alignment characters
        content = content.replace('&', '')
        # Split at line breaks and clean
        lines = re.split(r'\\\\', content)
        cleaned = '\n\n'.join(f'$$\n{line.strip()}\n$$' for line in lines if line.strip())
        return cleaned

    # Process all $$...$$ blocks
    explanation = re.sub(r'\$\$(.*?)\$\$', split_multiline_block, explanation, flags=re.DOTALL)

    return explanation

def latexify_multiline_math_blocks(text: str) -> str:
    """
    Wrap multiline math blocks with $$...$$, remove alignment characters,
    preserve inline math and normal text without extra formatting,
    preserve blank lines as paragraph breaks.

    Strips leading/trailing spaces from normal lines to avoid code block rendering.
    """
    ...
    for line in lines:
        stripped = line.strip()
        # Detect multiline math lines by &= or line ends with \\
        if re.search(r'&=|\\\\$', stripped):
            ...
            if inside_math_block:
                ...
                inside_math_block = False
            # For normal lines:
            # If line is blank (only whitespace), append empty string to preserve blank line
            if line.strip() == '':
                result.append('')
            else:
                # Append normal line stripped of leading/trailing spaces to avoid code block triggers
...
def flatten_latex_matrices(text):
    # This regex matches \begin{...} ... \end{...} including newlines inside
    pattern = re.compile(r'(\\begin\{[^}]+\})(.*?)(\\end\{[^}]+\})', re.DOTALL)

    def replacer(match):
        begin, content, end = match.groups()
        # Remove all newline characters and excessive spaces inside content
        content_flat = ' '.join(content.split())
        return f"${begin} {content_flat} {end}$"
...
def demo():
    ...
    if not st.session_state.question_submitted:
        ...
        #answer = clean_latex_for_streamlit(answer)
        ...
        if not st.session_state.get("demo_answer_submitted", False):
            ...
            if user_answer is not None:
                ...
                #exp = flatten_latex_matrices(exp)
                exp = latexify_multiline_math_blocks(exp)
                st.info(exp)
                print(exp)
